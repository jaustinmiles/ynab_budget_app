...
export async function getBudgets(budgetIds: string[]): Promise<ICategoryMap> {
    ...
    for (const budgetId of budgetIds) {
        const budget = await getCachedYnabResponse([budgetId], CacheFunctions.GET_BUDGET_BY_ID) as ynab.BudgetDetailResponse;
...
export async function getBudgetCategoriesInMonth(categories: ICategories, month: string = "current"): Promise<Array<ynab.Category>> {
    const monthlyCategories: Array<ynab.Category> = []
    for (const category of categories.budgetCategories) {
        const monthlyCategory = await getCachedYnabResponse([categories.budgetId, month, category.id], CacheFunctions.GET_MONTH_CATEGORY_BY_ID) as ynab.CategoryResponse;
...
function buildReverseCategoryMapping(inputMap: YnabCategoryGroupMap, categories: ICategories) {
    ...
    for (const [group, categoryIds] of Object.entries(budgetMapping)) {
        for (const categoryId of categoryIds) {
            if (reverseMap.has(categoryId)) {
                throw new Error(`Duplicate category id ${categoryId} found in input map`);
            }
...
export function readCategoryGroups(inputMap: YnabCategoryGroupMap, categories: ICategories): IGroupedCategories {
