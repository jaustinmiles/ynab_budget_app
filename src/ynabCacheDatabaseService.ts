...
function createOrInitializeDb() {
    const db = new sqlite3.Database(process.env.YNAB_API_CACHE_DATABASE_URI);
...
export async function getCachedYnabResponse(params: string[], cacheFunction: CacheFunctions) {
    const db = YnabCacheDatabaseService.getDb();
    const paramsString = JSON.stringify(params);
    let updateNeeded = false
    const response = await new Promise((resolve, reject) => {
        db.serialize(() => {
            db.get(`SELECT response, last_requested
                    FROM api_cache
                    WHERE params = ?`, [paramsString], (err: any, row: any) => {
                if (err) {
                    reject(err);
                }
                ...
                if (cacheEntryRow && currentTime - cacheEntryRow.last_requested < oneHour) {
                ...
                try {
                    ...
                    switch (cacheFunction) {
                        ...
                        case CacheFunctions.GET_MONTH_CATEGORY_BY_ID:
                            const category = ynabApi.categories.getMonthCategoryById(params[0], params[1], params[2]);
    ...
    if (response) {
        ...
        if (updateNeeded) {
            db.serialize(() => {
                db.run(
                    'INSERT OR REPLACE INTO api_cache (params, response, last_requested) VALUES (?, ?, ?)',
